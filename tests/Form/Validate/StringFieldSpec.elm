        ...
        [ Test.describe "errorToMessage"
            [ Test.fuzz (Fuzz.intRange 20 29) "en-NZ mobile errors 20-29 - too short" <|
                \startPhoneNumber ->
                    let
                        ...
                        phoneNumber =
                            HelperSpec.phoneField (Just CountryCode.NZ) { required = IsRequired.Yes, value = startPhoneNumberStr ++ " 000" }
                    in
            ...
            , Test.test "en-NZ mobile too short - default prefix 20 - too short - not even a prefix" <|
                \_ ->
                    let
                        phoneNumber =
                            HelperSpec.phoneField (Just CountryCode.NZ) { required = IsRequired.Yes, value = "1" }
                    in
            ...
            , Test.test "en-NZ mobile too short - default prefix 20 - wrong prefix" <|
                \_ ->
                    let
                        phoneNumber =
                            HelperSpec.phoneField (Just CountryCode.NZ) { required = IsRequired.Yes, value = "31 123" }
                    in
                    StringField.errorToMessage phoneNumber Types.InvalidMobilePhoneNumber |> Expect.equal "Invalid mobile number (example: 2X XXX XXX[XX])"
            , Test.test "en-AU incorrect number" <|
                \_ ->
                    let
                        phoneNumber =
                            HelperSpec.phoneField (Just CountryCode.AU) { required = IsRequired.Yes, value = "400" }
                    in
            ...
            , Test.test "en-US incorrect number" <|
                \_ ->
                    let
                        phoneNumber =
                            HelperSpec.phoneField (Just CountryCode.US) { required = IsRequired.Yes, value = "212 3" }
                    in
            ...
            , Test.test "incorrect number with no country code" <|
                \_ ->
                    ...
                    in
                    StringField.errorToMessage phoneNumber Types.InvalidMobilePhoneNumber |> Expect.equal "Invalid mobile number (must specify a country code)"
            , Test.test "incorrect number for unknown locale" <|
                \_ ->
                    let
                        phoneNumber =
                            HelperSpec.phoneField (Just CountryCode.ES) { required = IsRequired.Yes, value = "925 295" }
                    in
